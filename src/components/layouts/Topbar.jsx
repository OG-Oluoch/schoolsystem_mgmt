...
function useBreadcrumbs() {
  ...
  segments.forEach((seg, i) => {
    path += `/${seg}`
    const label = ROUTE_LABELS[seg] ?? (seg.length > 20 ? 'Detail' : seg)
    crumbs.push({ label, to: path, current: i === segments.length - 1 })
  })
  return crumbs
}
...
export default function Topbar() {
  ...
  return (
    <header style={{
      height: 60, background: DS.surface, borderBottom: `1px solid ${DS.border}`,
      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      padding: '0 36px', flexShrink: 0, position: 'sticky', top: 0, zIndex: 10,
    }}>
      {/* Breadcrumb */}
      <nav style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: DS.textMuted }}>
        <span>🏫</span>
        <span>EduAdmin</span>
        {crumbs.slice(1).map((crumb) => (
          <span key={crumb.to} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {crumb.current
              ? <span style={{ color: DS.text, fontWeight: 700 }}>{crumb.label}</span>
              : <Link to={crumb.to} style={{ color: DS.textMuted, textDecoration: 'none' }}>{crumb.label}</Link>
            }
          </span>
        ))}
      </nav>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <span style={{ fontSize: 13, color: DS.textMuted }}>{now}</span>
        <div style={{ display: 'flex', gap: 6 }}>
          {['🔔', '⚙️', '❓'].map((icon, i) => (
            <button key={i} style={{
              width: 34, height: 34, borderRadius: 9, background: DS.bg,
              border: `1px solid ${DS.border}`, fontSize: 15, cursor: 'pointer',
